...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        b, t, c = x.size()
        if c != self.d_model:
            raise ValueError(f"输入最后一维 {c} 应等于 d_model={self.d_model}")
        if t > self.block_size:
            raise ValueError(f"序列长度 {t} 超过 block_size={self.block_size}")

        # 1. 线性映射得到 Q/K/V，并拆成多头：(B, T, C) -> (B, n_head, T, d_head)
        ...
        # 2. 缩放点积注意力分数：(B, n_head, T, T)
        att = (q @ k.transpose(-2, -1)) * (self.d_head ** -0.5)

        # 3. 因果掩码：屏蔽 j > i 的未来位置，防止模型偷看答案
        causal_mask = torch.triu(
            torch.ones(t, t, dtype=torch.bool, device=x.device), diagonal=1
        )
        att = att.masked_fill(causal_mask, float("-inf"))

        # 4. softmax 得到注意力权重，与 V 相乘，再合并多头
